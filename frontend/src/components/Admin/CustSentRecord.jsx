...
const ArchitectureManager = () => {
  ...
  const [showUserProfile, setShowUserProfile] = useState(false);
  const [selectedUserId, setSelectedUserId] = useState(null);
  const [userProfileData, setUserProfileData] = useState(null); // NEW STATE for user profile data
  ...
  const searchArchitecturesByName = (searchTerm) => {
    ...
    console.log(`Searching for exact match: "${searchTermExact}" in student records only`);
    ...
    console.log(`Found ${results.length} exact matches in student records for "${searchTermExact}"`);
  ...
  const compressImageBase64 = (base64Str, maxWidth = 600, maxHeight = 600, quality = 0.7) => {
    return new Promise((resolve) => {
      try {
        ...
        img.onload = () => {
          try {
            ...
            if (compressedBase64.length > 30000) {
              console.log(`Image too large (${compressedBase64.length} chars), applying more compression...`);
            ...
            console.log(`Image compressed: ${base64Str.length} -> ${compressedBase64.length} chars`);
        ...
        console.error('Error in compressImageBase64:', err);
        resolve(base64Str);
      }
    });
  };
  ...
  const downloadSearchedArchitecturesExcel = async () => {
    ...
    try {
      ...
      if (tokenClassMap.size === 0 && searchResults.length > 0) {
        ...
        // For each token, try to find its architecture and class
              ...
              try {
                ...
                if (imageValue && !imageValue.startsWith('data:image')) {
                  try {
                    ...
                    console.error(`Error converting image URL for ${fieldLabel}:`, convError);
                ...
                if (imageValue && imageValue.length > 30000) {
                  console.log(`Image for ${fieldLabel} (token: ${token}) is too large (${imageValue.length} chars). Compressing...`);
                  ...
                  if (compressedValue.length <= 30000) {
                    ...
                    console.log(`Successfully compressed to ${imageValue.length} chars`);
                  } else {
                ...
                console.error(`Error processing image for ${fieldLabel}:`, error);
